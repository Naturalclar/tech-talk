import { docco } from 'react-syntax-highlighter/styles/hljs'
import { CodeSurfer } from 'mdx-deck-code-surfer'
import { Head } from 'mdx-deck'
export { swiss as theme } from 'mdx-deck/themes'
import { Meta, Layout } from '../../components'

<Head>
  <Meta title="CircleCI Artifacts によるレビューの効率化"
  description="CircleCI ユーザーコミュニティミートアップ #3 での発表"
  slug="review-efficiently-with-artifact"
  publishedAt={new Date("2019-04-23")}
  />
</Head>

<Layout>

### Circle CI Artifacts によるレビューの効率化

CircleCI ユーザーコミュニティミートアップ #3

<div>

#### https://circleci.connpass.com/event/123860/

<hr />

#### [@natural_clar](https://twitter.com/natural_clar)

</div>

</Layout>

---

<Layout>

## アーティファクト使ってますか？

</Layout>

---

<Layout>

### Circle CI Artifact

<hr />

- ビルドされた`.jar` ファイルや `.ipa` ファイルなどを格納できる。
- 指定されたパスの中身を S3 に格納。
- https://circleci.com/docs/2.0/artifacts/

</Layout>

---

<Layout>

### 静的サイトの Artifact

<hr />

- 静的サイトのプレビュー
- Storybook のプレビュー
- Test Coverage Report

</Layout>

---

<Layout>

### 毎回 Artifact を見に行くのは手間では？ :thinking_face:

<img src="../assets/ci-artifact.png" height="400" width="80%" />

</Layout>

---

<Layout>

## Klank

</Layout>

---

<Layout>

### Klank

<hr />

<img src="../assets/artifact-report.png" height="180" />

- https://github.com/Naturalclar/klank
- 指定パスの Artifact を PR に対してコメントする
- パッと見に行けるので効率がいい

</Layout>

---

<Layout>

## 使い方

</Layout>

---

<Layout>

- config.yml で `store_Artifacts` する

<img src="../assets/circleci_config.png" height="380" width="340" />

</Layout>

---

<Layout>

- Project の環境変数に `CIRCLE_TOKEN`, `GITHUB_TOKEN` をいれる

<img src="../assets/circleci_env.png" height="380" />

</Layout>

---

<Layout>

- npm scripts に Artifact のパスをいれる

<img src="../assets/circleci_npm.png" height="280" width="400" />

</Layout>

---

<Layout>

## 実践デモ

https://github.com/Naturalclar/expo-typescript-starter

</Layout>

---

<Layout>

### セキュリティ

- Private Repo は Circle CI アカウントによって保護
- Circle CI にログインしていなければ閲覧不可

</Layout>

---

<Layout>

### 自己紹介

<img src="../assets/cat.jpg" height="150" width="150" />

- Jesse Katsumata アメリカ人 :flag-us:
- CureApp - React Native を使った医療系のアプリの開発
- OSS の翻訳活動もしています


##### Twitter: [@natural_clar](https://twitter.com/natural_clar) Github: [@Naturalclar](https://github.com/Naturalclar)

</Layout>

---

<Layout>

## ありがとうございました

</Layout>
