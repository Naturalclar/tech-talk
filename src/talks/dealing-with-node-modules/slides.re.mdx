...
{/* node_modules のブラックホールとの向き合い方について発表致します。Jesseです。よろしくお願い致します。 */}
...
<img src="../assets/black_hole.jpg" width="700" />
...
{/* node_modules の削減に力を入れることにはいくつかメリットがあります。 まず、当たり前な話ですが、インストール時間が削減できます。 GitHub Actions のような従量制課金の CI サービスを使っている場合、GitHub Actions が走るたびに node_modules がインストールされることになるので */}
...
<img src="../assets/ncdu.png" width="900" />

{/* 自分はnode_modulesの使用容量を見るためにncduというツールを使っています。 画像はとあるプロジェクトで使っているmonorepoの使用容量です。 このmonorepoではreact-nativeによるモバイルアプリ、react製のwebアプリ、node.js製のサーバーやaws-cdkを使ったインフラ構成が含まれているプロジェクトでnode_modulesの容量が1.4GBまで抑えられています。 packagesに含まれているのはmonorepoの子パッケージでこの中にnode_modulesは含まれておらず、容量を食っているのはiOSとAndroidのビルド成果物です。 このプロジェクトは元からこうだったわけではなく、もともとは 4GB 近い node_modules だったものを削減していきました。 CI も毎回５分かかっていたところが１分半で済むようになりました。 */}
...
{/* node_modulesの中には、`.npmignore` や package.json の files が適切に設定されておらず、docsやexample等が残ったままpublishされているパッケージがいくつかあります。これらは自分が開発している時にnode_modules内から使うことはほぼ無いので、容量の無駄になっています。 yarn を使っている場合、yarn autoclean というコマンドがあり、node_modulesに含まれる指定のフォルダを削除することができます。 .yarnclean 内に記載されているフォルダは消去されます。 node_modules の中の .gitignore の様なものです。 */}
