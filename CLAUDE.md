...
**Hosted on GitHub Pages**, built and deployed by `.github/workflows/ci.yml` on every push to `master`. It was on Netlify until #84; nothing in the build or the configuration references Netlify any more, and a `_redirects` file would do nothing. `create-your-own-slides-page` still names it, but that deck is a talk _about_ deploying with it — deck content, not infrastructure, the same distinction as the CodeSurfer mention below.
...
1. Wipe `dist/`, then copy `src/talks/assets/**` into it, and `src/favicon.svg` to `dist/favicon.svg`. **Assets go first** — decks reference shared images as plain `../assets/*` paths, which only resolve once `dist/assets` exists, and the screenshots in stage 3 would otherwise capture broken images. The favicon goes to the **root** rather than into `dist/assets/`: that directory answers on `/assets/`, and both shells ask for `/favicon.svg`.
2. `vite build` → `dist/<slug>/`, decks in parallel. Each is a build of the shared shell in `src/deck`, pointed at that deck's slides by the `DECK` variable that `vite.config.ts` reads. A deck that fails is dropped from the remaining stages and makes the build exit non-zero.
   ...
   **The favicon is the one deliberate exception.** Both shells link it as `/favicon.svg`, absolute, which is a decision taken knowing what it costs: served from a sub-path the pages still render, but the icon 404s, because the file is at `/tech-talk/favicon.svg` and the link says `/favicon.svg`. A relative `href` cannot be written once for both shells — the landing page sits at the root and a deck one directory down, so the same string cannot reach the same file from both. The custom domain is the address that matters; the sub-path is a fallback that loses its tab icon and nothing else.
...
**A tag that exists is not an image that loads**, so the address is checked with a `HEAD` before it is used (a `405` is retried as a cancelled `GET`, since some hosts refuse the method). Decks bake an _absolute_ `og:image` into their HTML at build time, so one that later moved host still advertises the address it had then — the three talks this was written for were 2019 decks still naming `now.sh` from pages that had moved to `vercel.app`. Those three are no longer listed, having been migrated into this repository, but the situation is the normal one for a page nobody here controls. When the advertised image does not load, the filename is tried again where the deck actually lives now — **beside the page first, then at the root of the host** — because the image is almost always an asset of the deck itself. The directory comes first because it is what survives a move into a sub-path; the root is what a deck with a subdomain to itself looks like, which is the shape of the older talks. If both fail, the card falls back to the placeholder rather than shipping a broken `<img>`.
...
The thumbnails are `loading="lazy"` — the listing is thirty cards and about two rows are on screen, so without it every visitor downloaded all thirty screenshots, 1.09MB, before seeing any of the page. Measured on the built page, fifteen images and 434KB now wait until they are scrolled to. Nothing jumps when they arrive: the box is `aspect-video`, the same size before the request finishes, after it fails, and for the placeholder.

**They are deliberately not `decoding="async"`, and adding it breaks the build's reproducibility.** Asynchronous decoding lets a thumbnail be painted a frame after it is laid out, and stage 8 photographs this page — so the shutter caught the bottom edge of one thumbnail mid-paint, thirty pixels of a four-row band, differently each run. `dist/index.png` came out 71533, 71518 and 71460 bytes over three builds, against a `dist` that is otherwise byte-identical. Waiting the images out in `generate-screenshot.ts` was tried three ways and none of them closed the race; not asking for the decode to be asynchronous does, in one word, and there is nothing to weigh against it for thirty small PNGs whose download `loading="lazy"` already handles.
